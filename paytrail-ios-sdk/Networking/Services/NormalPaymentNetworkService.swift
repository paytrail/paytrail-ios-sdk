...
final class NormalPaymentNetworkService: NetworkService {
    ...
    func request<Request: DataRequest>(_ request: Request, completion: @escaping (Result<Request.Response, PTError>) -> Void) {
        ...
        URLSession.shared.dataTask(with: urlRequest) { (data, response, error) in
            if let error = error {
                return completion(.failure(PTError(type: .createPayment, code: (response as? HTTPURLResponse)?.statusCode ?? nil, message: error.localizedDescription)))
            }
            ...
            guard let response = response as? HTTPURLResponse, 200..<300 ~= response.statusCode else {
                ...
                guard let error = data else {
                    let error = PTError(type: .createPayment, code: (response as? HTTPURLResponse)?.statusCode ?? nil, message: error?.localizedDescription)
                    return completion(.failure(error))
                }
                
                do {
                    let decodedError = try jsonDecode(of: PaymentErrorResponse.self, data: error)
                    let paymentError = PTError(type: .createPayment, code: (response as? HTTPURLResponse)?.statusCode ?? nil, message: decodedError.localizedDescription)
                    ...
                    return completion(.failure(PTError(type: .jsonDecode, code: (response as? HTTPURLResponse)?.statusCode ?? nil, message: error.localizedDescription)))
                }
            }
            
            guard let data = data else {
                // Return failure when data is missing
                return completion(.failure(PTError(type: .createPayment, code: response.statusCode, message: nil)))
            }
            ...
            do {
                ...
                return completion(.failure(PTError(type: .jsonDecode, code: response.statusCode, message: error.localizedDescription)))
            }
